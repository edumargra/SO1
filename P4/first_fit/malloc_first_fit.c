...
void free(void *p) {
    //Si el punter és NULL, ignorem la crida
    if (p != NULL) {
        p_meta_dades meta_dades = (p_meta_dades*) p;
        //Comprovem que l'atribut magic contingui el valor correcte
        if (meta_dades->magic != MAGIC) {
            //Mostrem missatge d'error
        } else {
            //Posem l'atribut disponible de les meta_dades a 1.
